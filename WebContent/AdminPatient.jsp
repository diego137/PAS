...
<title>Administrador Patients</title>
    ...
    <div class="container" >
       ...
       <%--CONEXION A BASE DE DATOS CON SCRIPLET PARA MOSTRARLOS EN ESTE JSP --%>         
		...
		try
		{
			...
			{
			String sentenciaSQL = "select * from pacientes";  // nombre de la base de datos del paciente
			datos = stmnt.executeQuery(sentenciaSQL);
                ...
                <tr>
				 <td>
				  <span class="custom-checkbox">
					<input type="checkbox" id="checkbox1" name="options" class="paraEliminar" value=<%=datos.getInt("idPaciente") %>>
					<label for="checkbox1"></label>
				  </span>
				<td><%=datos.getInt("idPaciente") %></td>
				<td><%=datos.getString("nombrePaciente") %></td>
				<td><%=datos.getString("apellidosPaciente") %></td>
				<td><%=datos.getString("curpPaciente") %></td>
				<td><%=datos.getString("usuarioPaciente") %></td>
